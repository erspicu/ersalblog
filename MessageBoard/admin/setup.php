<?php
/**
 * MessageBoard Admin Setup - 風格統一版
 */
...
$success_msg = ''; $error = ''; $action = isset($_POST['action']) ? $_POST['action'] : '';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'save_global') {
        ...
        if (file_put_contents($jsPath, $content)) { $success_msg = "Settings updated!"; $_SESSION['mb_admin_mode'] = $newMode; }
        ...
        if (!empty($newPassword)) $finalPassword = password_hash($newPassword . getMBSystemFingerprint(), PASSWORD_BCRYPT);
        $newPhpContent = "<?php\n/**\n * MessageBoard Service - PHP Configuration\n */\n\n// 管理員帳號設定\n\$mb_admin_user = '" . addslashes($newUsername) . "';\n\$mb_admin_pass = '" . addslashes($finalPassword) . "';\n\n// 安全設定\n\$mb_session_name = 'MB_ADMIN_SESSION';\n";
...
function get_plugin_langs() {
    $langs = array(); $files = glob(__DIR__ . '/../langs/plugin-*.js');
...
<head>
    <meta charset="UTF-8"><title><?php echo __mb('menu_settings'); ?></title>
    <link href="assets/css/bootstrap.min.css" rel="stylesheet">
    <link rel="stylesheet" href="assets/css/bootstrap-icons.min.css">
    <style>.diag-item { border-bottom: 1px solid #e9ecef; padding: 12px 0; } .diag-item:last-child { border-bottom: none; }</style>
</head>
<body class="bg-light">
    <div class="d-flex">
                    ...
                    <div class="card border-0 shadow-sm mb-4">
                        ...
                        <div class="card-body">
                            <form method="POST"><input type="hidden" name="action" value="save_global">
                                <div class="mb-3"><label class="form-label small fw-bold"><?php echo __mb('label_mode'); ?></label>
                                ...
                                <div class="mb-3"><label class="form-label small fw-bold"><?php echo __mb('label_theme'); ?></label>
                                ...
                                <div class="mb-3"><label class="form-label small fw-bold"><?php echo __mb('label_plugin_lang'); ?></label>
                                ...
                                <div class="mb-3"><label class="form-label small fw-bold"><?php echo __mb('label_per_page'); ?></label>
                                    <input type="number" name="mb_per_page" class="form-control form-control-sm" value="<?php echo $currentPerPage; ?>" min="1" max="50">
                                </div>
                                <button type="submit" class="btn btn-primary btn-sm w-100"><?php echo __mb('label_save_global'); ?></button>
                            </form>
                        </div>
                    </div>
                    <div class="card border-0 shadow-sm">
                        ...
                        <div class="card-body">
                            <?php foreach($diagnostics as $d): ?>
                                <div class="diag-item"><div class="d-flex justify-content-between align-items-center mb-1"><small class="fw-bold"><?php echo $d['label']; ?></small><span class="badge <?php echo $d['pass'] ? 'bg-success' : 'bg-danger'; ?>"><?php echo $d['pass'] ? __mb('diag_pass') : __mb('diag_fail'); ?></span></div><div class="small text-muted"><?php echo $d['value']; ?></div></div>
                            <?php endforeach; ?>
                        </div>
                    </div>
                </div>
                <div class="col-lg-8">
                    <div class="card border-0 shadow-sm mb-4">
                        ...
                        <div class="card-body py-4">
                            <form method="POST"><input type="hidden" name="action" value="save_account">
                                <div class="row">
                                    <div class="col-md-6 mb-3"><label class="form-label small fw-bold"><?php echo __mb('label_new_username'); ?></label><input type="text" name="new_username" class="form-control" value="<?php echo htmlspecialchars($mb_admin_user); ?>" required></div>
                                    <div class="col-md-6 mb-3"><label class="form-label small fw-bold"><?php echo __mb('label_new_password'); ?></label><input type="password" name="new_password" class="form-control" placeholder="<?php echo __mb('hint_password_keep'); ?>"></div>
                                </div>
