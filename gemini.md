...
- **時區規範**: 所有時間記錄、日誌 (尤其是 `MD/gemini_log.md`) 與版本號必須統一使用 **UTC+8** 時區。在執行任何記錄動作前，應優先使用 Linux `date` 指令抓取當前系統時間，並務必確認並轉換為 UTC+8 時間。
- **後台開發**: 後台開發的部分都要考慮到多語支援，利用現在已經架構好得多語框架，不要把文字直接寫死在 code 裡。
- **相簿服務開發 (Album Service)**: 於 `album/` 目錄下發展獨立的相簿服務。開發階段以「完全獨立運行」為設計目標，確保其具備獨立的邏輯結構，後續再與 Blog 主服務整合。開發過程採循序漸進方式實作。
